...
static const uint8_t armedBitmapRLE [] = { 128, 32,
...
void bitmapDecompressAndShow(const uint8_t *bitmap)
{
    const uint8_t width = *bitmap++;
    const uint8_t height = *bitmap++;
    const uint16_t bitmapSize = (width * height) / 8;
    uint8_t data = 0;
    uint8_t count = 0;
    for (int ii = 0; ii < bitmapSize; ++ii) {
        if (count == 0) {
            data = *bitmap++;
            if (data == *bitmap) {
                // if data byte is repeated then next byte is repeat count
                count = *++bitmap;
                ++bitmap; // increment bitmap pointer for next time round loop
            } else {
                // data byte not repeated, so count is one
                count = 1;
            }
        }
        --count;
