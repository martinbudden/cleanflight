...
//! Private functions
static int32_t BMI160_Config(const sensorBus_t *bus);
static int32_t BMI160_do_foc(const sensorBus_t *bus);
static uint8_t BMI160_ReadReg(const sensorBus_t *bus, uint8_t reg);
static int32_t BMI160_WriteReg(const sensorBus_t *bus, uint8_t reg, uint8_t data);
 ...
 */
static void BMI160_Init(const sensorBus_t *bus)
{
