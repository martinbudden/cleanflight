...
extern float lastITermf[], ITermLimitf[];
extern biquad_t deltaFilterState[];
...
int16_t axisPID[FD_INDEX_COUNT];

#ifdef BLACKBOX
int32_t axisPID_P[FD_INDEX_COUNT];
int32_t axisPID_I[FD_INDEX_COUNT];
int32_t axisPID_D[FD_INDEX_COUNT];
int32_t axisPID_Setpoint[FD_INDEX_COUNT];
#endif

// Yaw axis lock accumulator
static float yawAxisLockAccum;
static pidState_t pidState[FD_INDEX_COUNT];
...
static void pidApplyHeadingLock(const pidProfile_t *pidProfile, pidState_t *pidState)
{
    ...
    if (ABS(pidState->rateTarget) > 2 || !ARMING_FLAG(ARMED)) {
        ...
        yawAxisLockAccum += (pidState->rateTarget - pidState->gyroRate) * dT;
        yawAxisLockAccum = constrainf(yawAxisLockAccum, -45, 45);
        pidState->rateTarget = yawAxisLockAccum * (pidProfile->P8[PIDMAG] / FP_PID_YAWHOLD_P_MULTIPLIER);
    }
}
...
static void pidLevel(const pidProfile_t *pidProfile, pidState_t *pidState, flight_dynamics_index_t axis, float horizonLevelStrength, uint16_t max_angle_inclination)
{
    // This is ROLL/PITCH, run ANGLE/HORIZON controllers
    const float angleTarget = pidRcCommandToAngle(rcCommand[axis]);
    const float angleError = (constrain(angleTarget, -max_angle_inclination, max_angle_inclination) - attitude.raw[axis]) / 10.0f;
...
static void pidApplyRateController(const pidProfile_t *pidProfile, pidState_t *pidState, flight_dynamics_index_t axis)
{
    ...
    // Calculate PTerm
    float PTerm = rateError * pidState->kP;
    ...
    if (pidProfile->D8[axis] == 0) {
        ...
        DTerm = -((2 * (pidState->dTermBuf[1] - pidState->dTermBuf[3]) + (pidState->dTermBuf[0] - pidState->dTermBuf[4])) / (8 * dT)) * pidState->kD;

        // Apply additional lowpass
        if (pidProfile->dterm_cut_hz) {
            // DTerm delta low pass filter
            DTerm = applyBiQuadFilter(DTerm, &deltaFilterState[axis]);
        }
    }

    // TODO: Get feedback from mixer on available correction range for each axis
    const float output = PTerm + lastITermf[axis] + DTerm;
    const float outputLimited = constrainf(output, -PID_MAX_TOTAL_PID, PID_MAX_TOTAL_PID);

    // Integrate only if we can do backtracking
    float ITerm = lastITermf[axis] + (rateError * pidState->kI + (outputLimited - output) * pidState->kT) * dT;
        ...
        uint16_t max_angle_inclination, const rollAndPitchTrims_t *angleTrim, const rxConfig_t *rxConfig)
{
    ...
    if (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE)) {
        ...
        pidLevel(pidProfile, &pidState[FD_ROLL], FD_ROLL, horizonLevelStrength, max_angle_inclination);
        pidLevel(pidProfile, &pidState[FD_PITCH], FD_PITCH, horizonLevelStrength, max_angle_inclination);
    }
